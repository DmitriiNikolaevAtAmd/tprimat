...
def validate_tensor_parallelism(model: str, tp: int) -> tuple[bool, str]:
    ...
    if num_heads % tp != 0:
        return False, f"❌ {specs['name']}: {num_heads} heads cannot be divided by TP={tp} (result: {num_heads/tp:.2f})"
    else:
...
def validate_parallelism_product(tp: int, pp: int, dp: int, num_gpus: int) -> tuple[bool, str]:
    ...
    product = tp * pp * dp
    if product != num_gpus:
        return False, f"❌ TP({tp}) × PP({pp}) × DP({dp}) = {product} ≠ {num_gpus} GPUs"
    else:
...
def validate_configuration(config, methodology: str, model: str, platform: str) -> list[tuple[bool, str]]:
    ...
    try:
        ...
        results.append((False, f"❌ Configuration not found: {e}"))
        return results
...
def main():
    ...
    if all_valid:
        ...
        print("❌ Some configurations have issues. Please fix config.yaml.")
        print()
