...
def normalize_env_paths(root: Path, keys: tuple[str, ...]) -> None:
    for key in keys:
...
def main() -> None:
    os.environ.setdefault("ROOT_DIR", str(WORKSPACE_ROOT))
    load_env_file(WORKSPACE_ROOT / "config.env")
    load_env_file(WORKSPACE_ROOT / "secrets.env")
    normalize_env_paths(WORKSPACE_ROOT, ("HF_HOME", "HF_DATASETS_CACHE"))
    sys.path.insert(0, str(WORKSPACE_ROOT))
    os.chdir(WORKSPACE_ROOT)

    parser = argparse.ArgumentParser(description="Run NeMo pretraining (llama/qwen)")
    parser.add_argument(
        "--model",
        default=os.environ.get("MODEL", "llama"),
        help="Model name (llama or qwen)",
    )
    parser.add_argument(
        "--platform",
        default=None,
        help="Override platform detection (nvd or amd)",
    )
    args = parser.parse_args()

    platform = args.platform.lower() if args.platform else detect_platform()
    if platform == "amd":
        from train import amd_nemo as nemo_impl
    else:
        from train import nvd_nemo as nemo_impl

    nemo_impl.train_model(args.model.lower())
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(description="Run Llama training using project defaults")
    parser.add_argument(
        "--model",
        default=os.environ.get("MODEL", "llama"),
        help="Model name (llama or qwen)",
    ...
    nemo_impl.train_model(model_name)
