...
def get_gpu_core_count(device_name: str, device_props) -> int:
    ...
    if hasattr(device_props, 'multi_processor_count'):
        return device_props.multi_processor_count * 64
...
def detect_gpu_info():
    ...
    if torch.cuda.is_available():  # Works for ROCm
        ...
        software_stack = "rocm" if is_rocm else "unknown"
        software_version = torch.version.hip if is_rocm else "unknown"
...
def extract_metrics_from_log(log_file, num_gpus, global_batch_size, seq_length, parallel_strategy=None, model_name=None):
    ...
    # Set platform based on software stack
    platform = "amd" if software_stack == "rocm" else "amd"
    ...
    # Always calculate from batch size and step time for consistency
...
Examples:
  ...
  python3 extract_primus_metrics.py \\
    ...
    if results:
        ...
        # Print next steps
        print("Next Steps:")
        print("  1. Run on AMD GPU and collect results")
