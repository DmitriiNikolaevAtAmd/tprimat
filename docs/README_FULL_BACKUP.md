...
python3 -u train_all_tran.py llama
torchrun --nproc_per_node=8 train_nvd_mega.py llama
...
```bash
# NVD
./train_docker_nvd.sh ./train_nvidia_all.sh
./train_docker_nvd.sh python3 -u train_all_tran.py llama

# AMD
./train_docker_amd.sh ./train_amd.sh
./train_docker_amd.sh python3 -u train_nemo_qwen.py
```
...
| Framework       | Identifier | Example                   | Description                           |
|-----------------|------------|---------------------------|---------------------------------------|
| Primus          | `prim`     | `train_amd_prim_llama.json`   | AMD Primus (ROCm optimized)           |
| NeMo            | `nemo`     | `train_nemo_llama.json`   | NVIDIA NeMo with ROCm support         |
| Transformers    | `tran`     | `train_tran_llama.json`   | HuggingFace Transformers with DDP     |
| DeepSpeed       | `deep`     | `train_deep_llama.json`   | Microsoft DeepSpeed with ZeRO-3       |
| FSDP            | `fsdp`     | `train_fsdp_llama.json`   | PyTorch Fully Sharded Data Parallel   |
| Megatron-DS     | `mgds`   | `train_mgds_llama.json` | Megatron-DeepSpeed (optional)         |
...
├── train_amd_prim_qwen.json     # Primus + Qwen 2.5 7B
├── train_nemo_llama.json    # NeMo + Llama 3.1 8B
├── train_nemo_qwen.json     # NeMo + Qwen 2.5 7B
├── train_tran_llama.json    # Transformers + Llama 3.1 8B
├── train_tran_qwen.json     # Transformers + Qwen 2.5 7B
├── train_deep_llama.json    # DeepSpeed + Llama 3.1 8B
├── train_deep_qwen.json     # DeepSpeed + Qwen 2.5 7B
├── train_fsdp_llama.json    # FSDP + Llama 3.1 8B
└── train_fsdp_qwen.json     # FSDP + Qwen 2.5 7B
```
...
| `amd-output.zip`    | All AMD `train_amd_prim_*.json`     |
...
```bash
python3 -u train_all_tran.py llama 2>&1 | tee output/hf_llama.log
```
...
|--------|---------|------------|--------------|
| `train_nvidia_all.sh` | All frameworks (NVD) | HF, NeMo | 4 files |
| `train_amd.sh` | All frameworks (AMD) | HF, NeMo | 4 files |
| `train_all_tran.py` | Transformers only | HF | 2 files | (shared) |
| `train_nvd_nemo_llama.py` | NeMo only | NeMo | 2 files |
| `train_prim.sh` | Prim (AMD-optimized) | Prim | 2 files |
| `train_prim.sh` | Prim direct | Prim | Logs + 2 files |
...
**What it does:** Same as `train_nvidia_all.sh` but with AMD-specific environment variables:
...
#### train_all_tran.py - Transformers Transformers
...
# Single model
python3 -u train_all_tran.py llama
python3 -u train_all_tran.py qwen
```
...
**What it does:**
1. Runs `train_amd_prim_llama.sh`
2. Runs `train_amd_prim_qwen.sh`
3. Provides summary with success/failure status

**Output:**
- `output/train_amd_prim_llama.json`
- `output/train_amd_prim_qwen.json`
...
- `output/train_amd_prim_llama.json` (extracted metrics)
- `output/train_amd_prim_qwen.json` (extracted metrics)
...
# Custom command
./train_docker_nvd.sh bash -c "python3 -u train_all_tran.py llama && ls -lh output/"
```
...
**Usage:** Same syntax as `train_docker_nvd.sh`
...
**AMD-Specific (set by `train_amd.sh`):**
...
./train_docker_amd.sh ./train_prim.sh

# Override HF token
HF_TOKEN=another_token ./train_docker_amd.sh
```
...
**For NVD:**
```bash
./train_docker_nvd_detached.sh
```

**For AMD:**
```bash
./train_docker_amd_detached.sh
```
...
# Run your benchmark
./train_nemo_all.sh  # or ./train_prim.sh
...
./train_docker_nvd.sh  # or ./train_docker_amd.sh
...
# Benchmark output
docker exec primat cat /workspace/tprimat/output/train_amd_prim_llama.json
```
...
├── train_amd_prim_llama.json        # AMD Llama metrics (if available)
├── train_amd_prim_qwen.json         # AMD Qwen metrics (if available)
...
- **Examples**:
  ...
  - `train_amd_prim_llama.json` (Primus + Llama on AMD)
...
```bash
./train_all_configs.sh [llama|qwen|both]
...
├── train_all_configs.sh        # Run all configurations
...
|-----------|--------|--------------|-------------|
| Transformers | `train_all_tran.py` | `train_tran_*.json` | Standard training, easy debugging (shared) |
| NeMo | `train_nvd_nemo_llama.py` | `train_nemo_*.json` | Advanced features, FP8, best performance |
| Prim (AMD) | `train_prim.sh` | `train_amd_prim_*.json` | AMD-optimized, MI300X |
...
├── train_amd_prim_llama.json      # Prim Llama (AMD)
└── train_amd_prim_qwen.json       # Prim Qwen (AMD)
```
