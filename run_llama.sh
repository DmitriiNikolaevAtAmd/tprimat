...
# Load project defaults (optional).
...
# Minimal runtime defaults for NeMo runners.
...
set -x
python3 -u "$ROOT_DIR/run_llama.py" --model "$MODEL" ${PLATFORM:+--platform "$PLATFORM"}
#!/bin/bash
set -e
...
set -x
python3 -u "$ROOT_DIR/run_llama.py" --model "$MODEL"
