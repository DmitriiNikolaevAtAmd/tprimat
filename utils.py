...
class BenchmarkCallback(Callback):
    ...
    def on_train_start(self, trainer, pl_module):
        ...
        if self.profiler_enabled and trainer.is_global_zero and torch.cuda.is_available():
            ...
            # Create profiler output directory (save directly in output_dir)
            profiler_dir = self.output_dir
            profiler_dir.mkdir(parents=True, exist_ok=True)
            
            # Generate filename with model name (no timestamp)
            software_stack = self.gpu_info.get("software_stack", "unknown")
            if self.model_name:
                trace_prefix = f"profile_{software_stack}_{self.model_name}_{self.parallel_strategy}"
            else:
                trace_prefix = f"profile_{software_stack}"
    ...
    def on_train_end(self, trainer, pl_module):
        ...
        if self.profiler is not None:
            try:
                ...
                # Rename the profile file to remove timestamps/IDs if possible
                software_stack = self.gpu_info.get("software_stack", "unknown")
                if self.model_name:
                    trace_prefix = f"profile_{software_stack}_{self.model_name}_{self.parallel_strategy}"
                else:
                ...
                # Find the file generated by tensorboard_trace_handler
                # It usually looks like trace_prefix.TIMESTAMP.pt.trace.json
                for p_file in self.output_dir.glob(f"{trace_prefix}*.pt.trace.json"):
                    # Only rename if it's not already the desired name
                    target_name = f"{trace_prefix}.pt.trace.json"
                    if p_file.name != target_name:
                        target_path = self.output_dir / target_name
                        # If target already exists, remove it first
                        if target_path.exists():
                            target_path.unlink()
                        p_file.rename(target_path)
                        print(f"✓ Profile renamed to: {target_name}")
                        break
                ...
                if self.profiler_config.get('export_chrome_trace', True):
                    ...
                    print(f"✓ Profiler traces saved to: {self.output_dir}")
                    print(f"  View in TensorBoard: tensorboard --logdir={self.output_dir}")
                    print(f"  Or Chrome: chrome://tracing (load .json files)")
            except Exception as e:
