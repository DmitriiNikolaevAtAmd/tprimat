...
def fetch_bookcorpus(num_samples: int, output_file: str, max_retries: int = 3):
    from datasets import DownloadConfig
    import time
...
def main():
    parser = argparse.ArgumentParser(description="Fetch datasets and tokenizers")
    parser.add_argument(
        "--samples",
        type=int,
    ...
    parser.add_argument(
        ...
        help="Output JSONL file path for C4",
    )
    parser.add_argument(
        "--bookcorpus-output",
        type=str,
        default=f"{DATA_DIR}/bookcorpus-raw.jsonl",
        help="Output JSONL file path for BookCorpus",
    )
    ...
    fetch_bookcorpus(args.samples, args.bookcorpus_output)
