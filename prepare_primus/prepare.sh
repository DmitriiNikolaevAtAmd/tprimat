#!/bin/bash
###############################################################################
# Master Data Preparation Script for Primus
#
# This script orchestrates the download and preprocessing of datasets
# for training with different backends (Megatron, MaxText, TorchTitan).
#
# Usage:
#   ./prepare.sh --help                    # Show help
#   ./prepare.sh --all                     # Prepare everything
#   ./prepare.sh --dataset bookcorpus      # Download BookCorpus only
#   ./prepare.sh --dataset c4              # Download C4 samples only
#   ./prepare.sh --tokenizer llama3-8b     # Download Llama 3 tokenizer
#   ./prepare.sh --tokenize                # Tokenize downloaded data
#
# Environment variables:
#   HF_TOKEN    - HuggingFace token (required for some models)
#   DATA_DIR    - Data directory (default: ./data)
#   WORKERS     - Number of workers for tokenization (default: auto)
###############################################################################
...
prepare_all() {
    ...
    # Download default tokenizer (Llama 3)
